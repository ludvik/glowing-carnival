#!/usr/bin/env python3
"""Validate the curated model screening-pool configuration.

This is intentionally lightweight: it checks that the curated screening pool is
present in the latest DigitalOcean Serverless Inference model snapshot and that
each selected model has enough local metadata for screening planning and later
cost accounting.
"""
...
SCREENING_POOL_PATH = Path("config/screening_pool.csv")
...
def main() -> int:
    screening_pool = load_screening_pool(SCREENING_POOL_PATH)
    ...
    screening_model_ids = [row["model_id"] for row in screening_pool]
    duplicate_ids = sorted({model_id for model_id in screening_model_ids if screening_model_ids.count(model_id) > 1})
    if duplicate_ids:
    ...
    missing_metadata = sorted(model_id for model_id in screening_model_ids if model_id not in metadata_models)
    if missing_metadata:
        raise SystemExit(f"Screening-pool models missing metadata: {', '.join(missing_metadata)}")
